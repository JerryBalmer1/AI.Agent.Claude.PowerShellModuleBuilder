...
<#
.SYNOPSIS
    Resolve every internal link in the documents pass 0040 touched, every doc
...
.EXAMPLE
    ...
    $report = try {
        ...
        ./plans/0040-flow-docs/Test-Links.ps1 @params
...
$Touched = @(
    ...
    'docs/creating-an-agent/00-start-here.md'
...
$mapped = @{}
foreach ($row in [regex]::Matches($section.Groups[1].Value, '(?m)^\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*\[[^\]]+\]\(([^)]+)\)\s*\|')) {
    $mapped[$row.Groups[1].Value] = $row.Groups[3].Value
}
$before = $dead.Count
foreach ($node in $graph.nodes) {
